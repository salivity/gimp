...
document.addEventListener("DOMContentLoaded", () => {
    ...
    videos.forEach(video => {
        video.setAttribute("poster", "/gimp/images/video/poster/default.svg")
    });
    ...
    const kebabCase = (str, { transliterate = false } = {}) => {
        ...
        if (transliterate) {
            // Strips accents/diacritics: "Café au Lait" -> "cafe-au-lait"
        ...
        return normalized
            // Split camelCase across Unicode letter boundaries (e.g., "iPhone", "доброПожаловать")
            .replace(/(\p{Ll}|\p{Lo})(\p{Lu})/gu, "$1-$2")
            // Split acronym sequences preceding normal words (e.g., "HTMLElement" -> "HTML-Element")
            .replace(/(\p{Lu}+)(\p{Lu}[\p{Ll}\p{Lo}])/gu, "$1-$2")
            // Separate letters from numbers (e.g., "item12" -> "item-12")
            .replace(/([\p{L}\p{Lo}])(\p{N})/gu, "$1-$2")
            .replace(/(\p{N})([\p{L}\p{Lo}])/gu, "$1-$2")
            // Replace non-alphanumeric separators (symbols, punctuation, whitespace) with hyphens
            .replace(/[^\p{L}\p{N}]+/gu, "-")
    ...
    const remapArticles = (data) => {
        return data.map((item) => {
            return { id: item[0], title: item[1], description: item[2], url: `/gimp/article/${kebabCase(item[1])}`, open_graph: item[3] };
        });
    };
    ...
    if (articlesContainer) {
        fetch("/gimp/js/articles.json").then(res => res.json()).then(data => {
    ...
    function renderList(items, container, paginatorId, page, isSearch, onPageChange) {
        ...
        pageItems.forEach(item => {
            ...
            if (item.open_graph === true) {
                ...
                imageTop.style.backgroundImage = `url(/gimp/images/open-graph/article/${item.id}.webp)`;
                ...
                imageBottom.src = `/gimp/images/open-graph/article/${item.id}.webp`;
